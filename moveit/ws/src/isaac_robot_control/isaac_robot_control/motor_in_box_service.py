"""
Piper Motor-in-Box 서비스 노드.

서비스 콜을 받으면 환경 리셋 → GreyCube를 WhiteBox에 넣기 → 결과 반환.
...
사용법 (MoveIt2 Docker 컨테이너 내부):
    ...
    ros2 run isaac_robot_control motor_in_box_service --ros-args -p pick_object:=GreyCube -p place_target:=WhiteBox
...
from isaac_robot_control.tasks import MotorInBoxTask
...
def main(args=None):
    ...
    service_node = rclpy.create_node("motor_box_service_server")
    ...
    service_node.create_service(Trigger, "/motor_in_box", service_cb)
    ...
    log.info("=" * 50)
    log.info(f"  Motor-in-Box 서비스 대기 중")
    log.info(f"  pick: {pick_object}, target: {place_target}")
    log.info(f"  호출: ros2 service call /motor_in_box std_srvs/srv/Trigger")
    log.info("=" * 50)
    ...
    try:
        while rclpy.ok():
            ...
            log.info("[DEBUG] 메인 스레드: Motor-in-Box 시작")
            task = MotorInBoxTask(
