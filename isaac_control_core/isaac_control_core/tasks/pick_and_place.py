...
class PickAndPlaceTask(BaseTask):
    ...
    def __init__(
        ...
        """
        Args:
            grasp_yaw: 그리퍼 yaw 설정 (물체 yaw 기준 상대 각도, 도 단위).
                - "auto": 오브젝트 X축에 정렬 (물체 따라 회전)
                - 0: 로봇 기본 그리퍼 자세 그대로 (Z 회전 없음) — 물체 orientation=0 인 경우 추가 회전 없음
                - 90: Z축 90° 회전 grip
                - "horizontal": 0과 동일
                - "vertical": 90과 동일
                - float: 임의 각도(도)
    ...
    def execute(self) -> bool:
        ...
        # grasp yaw 계산
        # yaw=0 + obj_yaw=0 → Z 회전 0 (로봇 기본 그리퍼 자세 그대로, 회전 없음).
        pick_ori_kwargs = {}
        grasp_yaw_rad = None
        offset_rad = None
        ...
        if offset_rad is not None:
            ...
            # obj_yaw 변동 시에도 같은 user intent 에 대해 연속적인 joint6 target 보장 (wrist flip 방지).
            while grasp_yaw_rad - offset_rad > math.pi / 2:
                grasp_yaw_rad -= math.pi
            while grasp_yaw_rad - offset_rad < -math.pi / 2:
                grasp_yaw_rad += math.pi

            # 평행 그리퍼 target 대칭 (target Z-rot ± π 는 동일 grasp) 을 이용해 |final_z_rot| 최소화.
            # joint6 회전량 감소.
            final_z_rot = grasp_yaw_rad
